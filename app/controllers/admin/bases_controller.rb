class Admin::BasesController < AdminController
  ...
  def create
    ...
    respond_to do |format|
      if @basis.save
        format.html { redirect_to [:admin, @basis], notice: 'Base was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @basis.update(basis_params)
        format.html { redirect_to [:admin, @basis], notice: 'Base was successfully updated.' }
        format.json { render :show, status: :ok, location: [:admin, @basis] }
      else
  ...
  private
    ...
    def basis_params
      params.require(:base).permit(:color, :image, :hoop, :neck)
    end
end
